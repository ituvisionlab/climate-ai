...
for p in range(prediction_month):
    for year in all_years:
        for month in all_months:
            ...
            np.save(template_path+'/all_targets.npy', output_tensor, allow_pickle=True)
                        
            #sharpness_per_pixel = sharpness2d(np.asarray(std_pred))
            ...
            # RMSE XARRAY
            pred_err_map2 = np.square(preds - output_tensor) ** 0.5
            ...
            fig_num1 = 1
            data1 = np.mean(preds, axis=(0)) - 273.5 # Take mean accross all samples and convert to Degree from Kelvin
