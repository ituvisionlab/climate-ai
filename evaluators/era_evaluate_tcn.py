    ...
    for year in all_years:
        for month in all_months:
            ...
            for i in range(val_input.shape[0]):
                preds = model(torch.flatten(torch.unsqueeze(val_input[i], dim=0), start_dim=2))
                preds = preds.reshape(-1, 192, 288)
                preds = preds.detach().cpu().numpy() * attribute_norm_vals[attribute_name][1] + attribute_norm_vals[attribute_name][0]
                all_mean_preds.append(preds)
            
            preds = np.concatenate(all_mean_preds, axis=0)
            mean_preds = np.mean(preds, axis=0)
            all_std_preds = np.std(preds, axis=0)
            np.savetxt(template_path+"/means.txt", mean_preds)
            np.savetxt(template_path+"/stds.txt", all_std_preds)
            
            nll, sharpness_per_pixel, mae, rmse = compute_metrics(preds, np.expand_dims(all_std_preds,axis=0), output_tensor)
