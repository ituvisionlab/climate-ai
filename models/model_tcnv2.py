...
class Refine2dNet(M.Model):
	def initialize(self, temp_length, output_dimension=1):
		...
		self.c4 = M.ConvLayer1D(1, self.output_dimension)
	...
	def forward(self, x, drop=True):
		...
		x = self.c4(x)
		x = x.permute(0, 2, 1)
		x = x.reshape(x.shape[0], self.output_dimension, x.shape[2], x.shape[3])
		return x 
